        ...
        for record in mongo_db.collection.find():
            ...
            for file_name in file_list:
                ...
                if file_name.endswith('.doc') or file_name.endswith('.docx'):
                    os.system(path_commond + 'unoconv -f txt %s' % file_name)
                    os.system(path_commond + 'mv %s.txt %s' % (base_name, trans_path))
                elif file_name.endswith('.xls') or file_name.endswith('.xlsx'):
                    os.system(path_commond + 'unoconv -f txt %s' % file_name)
                    os.system(path_commond + 'mv %s.txt %s' % (base_name, trans_path))
                elif file_name.endswith('.pdf'):
                    os.system(path_commond + 'pdftotext -pgnobrk %s %s/%s.txt' % (file_name, trans_path, base_name))
                # 压缩文件类型不齐全
                # 目前包括 rar zip gz
                    ...
                    logger.info('trying trans file with unconv')
                    result = os.system(path_commond + 'unoconv -f txt %s' % file_name)
                    if not result:
                        os.system(path_commond + 'mv %s.txt %s' % (base_name, trans_path))
                        continue
                    else:
                        logger.warn('trans file with unconv failed')
                    logger.info('trying trans file with pdftotext')
                    result = os.system(path_commond + 'pdftotext -pgnobrk %s %s/%s.txt' % (file_name, trans_path, base_name))
                    if not result:
                        continue
                    else:
